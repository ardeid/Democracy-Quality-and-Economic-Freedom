...
Gini_Koeffizent = Gini_Koeffizent %>% select(!c(`Indicator Name`, `Indicator Code`, "1960", "1961", "1963")) %>% 
  pivot_longer(cols = -c(`Country Name`, `Country Code`), names_to = "Year", values_to = "Gini_Index") %>% filter(Gini_Index !=is.na(TRUE))
...
Weltbank_Indicators = Weltbank_Indicators %>% pivot_longer(cols = !c(`Series Name`, `Series Code`, `Country Name`, `Country Code`)) %>% 
  separate_wider_delim(cols = name, delim = " ", names = c("Year", "1")) %>% 
select(`Country Name`, `Country Code`, Year, `Series Name`, value) %>% 
  mutate(value = as.numeric(value), Year = as.numeric(Year)) %>% filter(!is.na(`Country Code`)) %>% pivot_wider(names_from = `Series Name`, values_from = value) %>% 
...
gdppc_Wordbank = gdppc_Wordbank %>% pivot_longer(cols = !c(`Series Name`, `Series Code`, `Country Name`, `Country Code`)) %>% 
  ...
  select(`Country Name`, `Country Code`, Year, `Series Name`, value) %>% 
  mutate(value = as.numeric(value), Year = as.numeric(Year)) %>% filter(!is.na(`Country Code`)) %>% pivot_wider(names_from = `Series Name`, values_from = value) %>%
  rename("GDPpc_WB" = `GDP per capita (constant 2015 US$)`) %>% select(!`Country Name`)
...
efotw_1970_2021 = efotw_1970_2021 %>% rename("EconFree_Index" = `Economic Freedom Summary Index`)
#Auswahl aus efotw_1970_2021 treffen
EconFreeOTWorld = efotw_1970_2021 %>% select(Year, `ISO Code 2`, `ISO Code 3`, Countries, EconFree_Index)
...
efotw_1950_1965 = efotw_1950_1965 %>% rename("Countries" = Country, "EconFree_Index" = EFW)
...
EconFreeOTWorld = EconFreeOTWorld %>% filter(EconFree_Index != is.na(TRUE))

#efotw_1950-1965 hat keine ISO Code 3 Spalte. Diese zeile füllt die NA´s mit den passenden Ländercodes auf.
EconFreeOTWorld = EconFreeOTWorld %>% group_by(Countries) %>% fill(`ISO Code 3`, .direction = "down")
...
V_Dem_v13 <- vdem
#str(V_Dem_v13)
V_Dem_subset = V_Dem_v13 %>% select(country_name, country_text_id, year, v2x_libdem, v2x_liberal, v2x_polyarchy, v2x_partip, v2xdl_delib, v2x_egal, 
                                    e_gdppc, e_pop, e_regionpol, e_chga_demo)
...
#Daten einladen mit dem WID R Package.
inc = download_wid(indicators = "sptinc", areas = "all", years = 1950:2022, perc = c("p0p50", "p90p100"), ages = 992, pop = "j")
wel = download_wid(indicators = "shweal", areas = "all", years = 1950:2022, perc = c("p0p50", "p90p100"), ages = 992, pop = "j")

#Variablen umformen
inc = inc %>% pivot_wider(names_from = percentile, values_from = value) %>% rename("Income_Top10%_%" = p90p100, "Income_Bottom50%_%" = p0p50) %>% select(!variable) %>% mutate(Income_10by50 = `Income_Top10%_%`/`Income_Bottom50%_%`)
wel = wel %>% pivot_wider(names_from = percentile, values_from = value) %>% rename("Wealth_Top10%_%" = p90p100, "Wealth_Bottom50%_%" = p0p50) %>% select(!variable) %>% mutate(Wealth_10by50 = `Wealth_Top10%_%`/`Wealth_Bottom50%_%`)
#Variablen zusammenfügen
WID_Data = full_join(wel, inc, join_by(x$year == y$year, x$country == y$country))
...
df_all_cases= full_join(V_Dem_subset, EconFreeOTWorld, join_by(x$year == y$Year, x$country_text_id == y$`ISO Code 3`))
df_all_cases = full_join(df_all_cases, Gini_Koeffizent, join_by(x$year == y$Year, x$country_text_id == y$`Country Code`))
df_all_cases = full_join(df_all_cases, WID_Data, join_by(x$year == y$year, x$`ISO Code 2` == y$country))
df_all_cases = full_join(df_all_cases, Weltbank_Indicators, join_by(x$year == y$Year, x$country_text_id == y$`Country Code`))
df_all_cases = full_join(df_all_cases, gdppc_Wordbank, join_by(x$year == y$Year, x$country_text_id == y$`Country Code`))

#Filtern des df nach Fehlenden Werten, Entfernen von Irrelevanten Variablen.
df_all_cases = df_all_cases %>% select(!c(`ISO Code 2`, Countries, `Country Name.x`, `Country Name.y`)) %>% filter(!is.na(EconFree_Index), !is.na(v2x_liberal))
...
rm(EconFreeOTWorld, V_Dem_subset, WID_Data, Gini_Koeffizent, Weltbank_Indicators, gdppc_Wordbank)
